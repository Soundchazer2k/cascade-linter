...
from .core import CodeQualityRunner, LinterStage
from .cli import (
    CLIInterface,
    CLIProgressCallback,
)  # Import existing CLI for compatibility
...
class EnhancedCLIProgressCallback(CLIProgressCallback):
    ...
    def _stage_finish_func(self, stage_name: str, success: bool, duration: float):
        ...
        self._log_event(
            "stage_finish", stage=stage_name, success=success, duration=duration
        )
        if self.verbose and not self.json_output:
...
class EnhancedCLIInterface(CLIInterface):
    ...
    def _create_enhanced_parser(self) -> argparse.ArgumentParser:
        ...
        dep_group.add_argument(
            "--export-csv", metavar="FILE", help="Export module breakdown as CSV file"
        )

        dep_group.add_argument(
            "--no-coverage", action="store_true", help="Skip test coverage integration"
        )
    ...
    def _run_enhanced_linting(self, parsed_args) -> int:
        ...
        progress_callback = EnhancedCLIProgressCallback(
            verbose=parsed_args.verbose, json_output=is_json_output
        )
        ...
        try:
            ...
            # Handle JSON output
            if is_json_output:
                return self._output_json_results(
                    session, parsed_args, progress_callback
                )
    ...
    def _run_parallel_linting(self, runner, parsed_args, stages, progress_callback):
        ...
        if other_stages:
            if not parsed_args.json and not parsed_args.json_pretty:
                print(
                    f"Running {len(other_stages)} stages in parallel: {', '.join(s.command for s in other_stages)}"
                )
                ...
                for future in as_completed(future_to_stage):
                    ...
                    try:
                        ...
                        if not parsed_args.json and not parsed_args.json_pretty:
                            ...
                            print(
                                f"   {status} {stage.command} ({result.execution_time:.2f}s)"
                            )
                    except Exception as e:
                        if not parsed_args.json and not parsed_args.json_pretty:
    ...
    def _output_json_results(self, session, parsed_args, progress_callback) -> int:
        ...
        for file_path, file_issues in session.issues_by_file.items():
            result_data["files"][file_path] = {
                ...
                "severities": {
                    "error": len(
                        [i for i in file_issues if i.severity.severity_name == "error"]
                    ),
                    "warning": len(
                        [
                            i
                            for i in file_issues
                            if i.severity.severity_name == "warning"
                        ]
                    ),
                    "info": len(
                        [i for i in file_issues if i.severity.severity_name == "info"]
                    ),
                },
    ...
    def _display_timing_info(self, session):
        ...
        stage_times = [
            (stage.command, result.execution_time)
            for stage, result in session.stage_results.items()
        ]
    ...
    def _handle_dependency_analysis(self, parsed_args) -> int:
        ...
        try:
            ...
            result = analyzer.analyze_codebase(
                include_mypy=include_mypy, include_coverage=include_coverage
            )
            ...
            if getattr(parsed_args, "export_graph", None):
                ...
                with open(parsed_args.export_graph, "w") as f:
                    f.write(dot_content)
                print(
                    f"{symbols.SUCCESS} Dependency graph exported to {parsed_args.export_graph}"
                )
                print(
                    f"{symbols.INFO} Render with: dot -Tpng {parsed_args.export_graph} -o graph.png"
                )
                return 0
                ...
                with open(parsed_args.export_csv, "w") as f:
                    f.write(csv_content)
                print(
                    f"{symbols.SUCCESS} Module breakdown exported to {parsed_args.export_csv}"
                )
                return 0
                ...
                show_details = getattr(parsed_args, "show_details", False) or getattr(
                    parsed_args, "debug_imports", False
                )
            ...
            print(
                f"{symbols.CRITICAL} Enhanced dependency analysis failed: {e}",
                file=sys.stderr,
            )
            if parsed_args.debug:
                import traceback
    ...
    def _handle_list_missing_docstrings(self, parsed_args) -> int:
        ...
        try:
            ...
            print()
            print(
                f"{symbols.INFO} Total files needing docstrings: {missing_docstrings['count']}"
            )
            ...
            print(
                f"{symbols.CRITICAL} Failed to scan for missing docstrings: {e}",
                file=sys.stderr,
            )
            if parsed_args.debug:
                import traceback
    ...
    def _generate_ai_recommendations(self, result, parsed_args) -> str:
        ...
        if result.priority_errors:
            ...
            critical_errors = [
                e for e in result.priority_errors if e["priority"] == "CRITICAL"
            ]
            ...
            if critical_errors:
                ...
                for error in critical_errors[:3]:  # Top 3
                    ...
                    lines.append(
                        "     Strategy: Fix type annotations and core interfaces"
                    )
            ...
            if high_errors:
                ...
                for error in high_errors[:3]:  # Top 3
                    ...
                    impact = error["impact_score"]
                    lines.append(f"   • {module} (affects {impact} modules)")
                    lines.append("     Strategy: Focus on public API consistency")

        if result.graph.cycles:
            lines.append(f"\n{symbols.CYCLE} Circular Dependency Resolution:")
            lines.append(
                "   Strategy: Break cycles by introducing interfaces or dependency injection"
            )
        ...
        if total_errors > 0:
            ...
            lines.append("   Priority approach could reduce this by 60-80%")

        lines.append(
            f"\n{symbols.SUCCESS} Note: AI recommendations available with --ai-recommendations"
        )
        lines.append(
            "   (This is algorithmic analysis - upgrade to AI for enhanced insights)"
        )
