...
class ThemeLoader:
    ...
    def load_theme(cls, theme_name: str, app: QApplication) -> bool:
        ...
        if theme_name not in cls.THEME_NAMES:
            cls.logger.warning(f"Unknown theme '{theme_name}', falling back to system")
            theme_name = "system"

        # System theme means no custom styling
        if theme_name == "system":
            app.setStyleSheet("")
            app.setProperty("currentTheme", "system")
            cls.logger.info("Applied system default theme")
            return True
        ...
        try:
            ...
            cls.logger.info(f"Successfully loaded theme: {theme_name}")
            return True

        except (FileNotFoundError, RuntimeError) as e:
            cls.logger.error(f"Failed to load theme '{theme_name}': {e}")
    ...
    def load_theme_by_index(cls, theme_index: int, app: QApplication) -> bool:
        ...
        if 0 <= theme_index < len(cls.THEME_NAMES):
            ...
            cls.logger.error(f"Invalid theme index: {theme_index}")
    ...
    def validate_themes_directory(cls) -> bool:
        ...
        if not themes_dir.exists():
            cls.logger.error(f"Themes directory not found: {themes_dir}")
            return False
        ...
        if missing_themes:
            cls.logger.warning(f"Missing theme files: {missing_themes}")
            return False
    ...
    def create_themes_directory(cls) -> bool:
        ...
        try:
            ...
            cls.logger.error(f"Failed to create themes directory: {e}")
            return False

    @classmethod
    def get_current_theme(cls, app: QApplication) -> Optional[str]:
        """Get the name of the currently applied theme."""
        return app.property("currentTheme") if app else None

    @classmethod
    def reload_current_theme(cls, app: QApplication) -> bool:
        """Reload the currently applied theme."""
        current_theme = cls.get_current_theme(app)
        if current_theme:
            return cls.load_theme(current_theme, app)
        return False
