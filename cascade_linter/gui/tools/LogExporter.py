...
class LogExporter:
    ...
    @staticmethod
    def export_to_json(
        log_data: Union[Dict[str, Any], List[Dict[str, Any]]], file_path: str
    ) -> bool:
        """
...
<body>
    ...
    def _format_data_as_html(data: Any) -> str:
        ...
        if isinstance(data, dict):
            ...
            for i, item in enumerate(data):
                html_lines.append(
                    f"<div>[{i}] {LogExporter._format_data_as_html(item)}</div>"
                )
            return "".join(html_lines)
        else:
