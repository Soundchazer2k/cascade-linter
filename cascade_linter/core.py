...
class IssueItem:
    ...
    def _is_ruff_fixable(self) -> bool:
        ...
        auto_fixable_prefixes = {
            'F401', 'F403', 'F405', 'F811', 'F841',  # Import issues
            'E501', 'E502', 'E701', 'E702', 'E703',  # Line length/formatting
            'W291', 'W292', 'W293', 'W391',          # Whitespace
            'E225', 'E231', 'E261', 'E262', 'E265',  # Spacing
...
class LintingSession:
    ...
    def finish(self):
        ...
        self.add_log_entry(f"Session completed in {self.execution_time:.2f}s")  # Removed emoji
        self.add_log_entry(f"Final summary: {self.total_issues} issues, {self.total_fixes_applied} fixes applied")
...
class CommandRunner:
    ...
    def run_command(self, command: List[str], description: str = "", 
                   ...
                   env_vars: Optional[Dict[str, str]] = None) -> Tuple[bool, str, str]:
        """Execute command with enhanced error handling and logging"""
        
        with self._lock:
            if self.logger:
                self.logger.info("command_start", 
            ...
            try:
                ...
                if self.debug:
                    print(f"Running command: {description or 'Running command'}")  # Removed emoji
                ...
                if self.logger:
                    self.logger.info("command_complete",
                                   success=success,
                ...
                error_msg = f"Command timed out after {self.timeout}s: {' '.join(command)}"
                if self.logger:
                    self.logger.error("command_timeout", command=" ".join(command), timeout=self.timeout)
                return False, "", error_msg
                
            except FileNotFoundError as e:
                error_msg = f"Command not found: {resolved_command[0]} (original: {command[0]})"
                if self.logger:
                    self.logger.error("command_not_found", command=command[0], error=str(e))
                return False, "", error_msg
                
            except Exception as e:
                error_msg = f"Unexpected error running {' '.join(command)}: {e}"
                if self.logger:
                    self.logger.error("command_error", command=" ".join(command), error=str(e))
                return False, "", error_msg
...
class IssueParser:
    ...
    PATTERNS = {
        ...
        'mypy': re.compile(r'^(.+?):(\d+):\s*(?:error|warning|note):\s*(.+?)(?:\s*\[([^\]]+)\])?$'),
        'pylint': re.compile(r'^(.+?):(\d+):(\d+):\s*([A-Z]\d+):\s*(.*)$'),
        'bandit': re.compile(r'>>.*?Issue:.*?Severity:.*?Test ID:.*?\n.*?Location:\s*(.+?):(\d+):(\d+)'),
    }
...
class RuffExecutor:
    ...
    def execute(self, path: str, check_only: bool = False, 
                unsafe_fixes: bool = False, 
                callback: Optional[LinterProgressCallback] = None) -> StageResult:
        ...
        try:
            ...
            if callback:
                callback.on_progress(f"Found {len(initial_issues)} initial issues")  # Removed emoji
            ...
            if not check_only and initial_issues:
                ...
                if callback:
                    callback.on_progress("Checking remaining issues...")  # Removed emoji
                ...
                if callback:
                    callback.on_progress(f"Applied {result.auto_fixes_applied} fixes")  # Removed emoji
    ...
    def _run_fix(self, path: str, unsafe: bool = False) -> bool:
        ...
        success, stdout, stderr = self.runner.run_command(
            cmd, 
            f"Ruff fix{'(unsafe)' if unsafe else ''}"
        )
        return success
...
class CodeQualityRunner:
    ...
    def __init__(self, debug: bool = False, simple_output: bool = False):
        self.debug = debug
    ...
    def run_linting_session(self, 
                          path: str,
                          stages: List[LinterStage] = None,
                          check_only: bool = False,
                          unsafe_fixes: bool = False,
                          callback: Optional[LinterProgressCallback] = None) -> LintingSession:
        ...
        session.add_log_entry(f"Starting cascade linting session")  # Removed emoji
        ...
        for stage in stages:
            if stage in self.executors:
                ...
                try:
                    ...
                    if callback:
                        callback.on_stage_finish(stage.command, result.success, stage_duration)
                    ...
                    session.add_log_entry(f"FAIL {stage.command} failed: {str(e)}")  # Removed emoji
                    if callback:
                        callback.on_stage_finish(stage.command, False, time.time() - stage_start)
    ...
    def _display_session_header(self, session: LintingSession):
        ...
        if self.console:
            try:
                ...
                self.print(f"CASCADE LINTER SESSION")
                self.print(f"Target: {session.target_path}")
                self.print(f"Stages: {' -> '.join(s.command for s in session.stages_requested)}")
    ...
    def _display_session_summary(self, session: LintingSession):
        ...
        if self.console:
            ...
            table = Table(title=f"Session Summary ({session.execution_time:.2f}s)")  # Removed emoji
        ...
        self.print(f"Total fixes applied: {session.total_fixes_applied}")  # Removed emoji
        self.print(f"Files with issues: {session.total_files_with_issues}")  # Removed emoji
        self.print(f"Overall success: {'YES' if session.success else 'NO'}")  # Removed emoji
    ...
    def generate_detailed_report(self, session: LintingSession) -> str:
        ...
        lines.append(f"Overall success: {'✅ PASSED' if session.success else '❌ ISSUES REMAIN'}")
        lines.append("")
        ...
        for stage in session.stages_requested:
            if stage in session.stage_results:
                result = session.stage_results[stage]
                lines.append(f"{stage.icon} {stage.command.upper()}: {result.status_icon}")
    ...
    def save_session_log(self, session: LintingSession, log_path: Optional[str] = None) -> str:
    ...
    def validate_environment(self) -> Dict[str, bool]:
        ...
        for stage in LinterStage:
            try:
                success, stdout, stderr = self.runner.run_command(
                    [stage.command, "--version"],
                    f"Checking {stage.command}"
                )
        ...
        if self.console:
            ...
            for stage in LinterStage:
                ...
                status_text = "✅ Available" if tools_status[stage.command] else "❌ Missing"
                table.add_row(
                    stage.command, 
...
def run_cascade_lint(path: str = ".",
    ...
    stage_objects = []
    if stages:
        stage_map = {stage.command: stage for stage in LinterStage}  # Use .command not .value
        for stage_name in stages:
            if stage_name in stage_map:
                stage_objects.append(stage_map[stage_name])
    else:
        stage_objects = [LinterStage.RUFF, LinterStage.FLAKE8]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Only check, don't auto-fix"
    )
    parser.add_argument(
        "--unsafe-fixes", action="store_true", 
        help="Apply Ruff's unsafe fixes"
    )
    parser.add_argument(
        "--debug", action="store_true",
        help="Enable debug output with local timestamps"
    )
    parser.add_argument(
