...
class ActionableReportFormatter:
    ...
    def format_actionable_console_report(self, result, show_details: bool = False) -> str:
        ...
        if not insights:
            lines.append(f"{symbols.SUCCESS} GREAT NEWS! No critical dependency issues found.")
            lines.append("")
            lines.append("Your project has a healthy dependency structure:")
            lines.append(f"  {symbols.ARROW} {result.total_files} files with clean import relationships")
            lines.append(f"  {symbols.ARROW} No circular dependencies detected")
            lines.append(f"  {symbols.ARROW} Well-modularized codebase")
            lines.append("")
            lines.append(f"{symbols.INFO} Consider running with MyPy analysis for error prioritization:")
            lines.append("  python -m cascade_linter.cli_enhanced --dependency-analysis .")
            return "\n".join(lines)
        ...
        for i, insight in enumerate(insights, 1):
            ...
            if insight.affected_files and show_details:
                lines.append(f"   Files affected: {', '.join(insight.affected_files[:3])}")
                if len(insight.affected_files) > 3:
                    lines.append(f"   ... and {len(insight.affected_files) - 3} more files")
    ...
    def _generate_actionable_insights(self, result) -> List[ActionableInsight]:
        ...
        for module, data in high_impact_modules:
            if data['imported_by_count'] >= 5:  # Used by 5+ modules
                insights.append(ActionableInsight(
                    ...
                    recommendation=f"Add comprehensive tests before modifying. Consider breaking into smaller, focused modules.",
                    affected_files=self._get_modules_that_import(module, result),
                    priority='HIGH' if data['imported_by_count'] >= 10 else 'MEDIUM'
                ))
        ...
        insights.sort(key=lambda x: (priority_order[x.priority], -len(x.affected_files)))
    ...
    def _identify_quick_wins(self, result) -> List[str]:
        ...
        if result.total_files > 20:
            wins.append("Add a project README with architecture overview to help new developers")
        
        if len(result.graph.cycles) > 0:
            wins.append("Document known circular dependencies to help future refactoring")
        
        # Check for potential improvements
        complex_count = len([m for m, d in result.metrics.items() 
        ...
        if result.total_imports > 200:
            wins.append("Review import statements - some might be unused or could be more specific")
    ...
    def _calculate_health_score(self, result) -> int:
        ...
        complex_modules = len([m for m, d in result.metrics.items() 
                             if isinstance(d, dict) and d.get('imports_count', 0) > 15])
        score -= min(complex_modules * 5, 25)
        
        # Deduct for high coupling
        high_coupling = len([m for m, d in result.metrics.items() 
    ...
    def _generate_next_steps(self, insights: List[ActionableInsight], result) -> List[str]:
        """Generate concrete next steps based on insights"""
        steps = []
        
        if not insights:
            steps.append("1. Great job! Run with MyPy integration to find type-related improvements")
        ...
        if critical_insights:
            steps.append(f"1. URGENT: Address {len(critical_insights)} critical issue(s) first")
        ...
        if result.total_errors == 0:
            steps.append("5. Add dependency analysis to CI/CD pipeline to prevent regressions")
...
def enhance_existing_formatter():
    ...
    try:
        ...
        DependencyAnalysisFormatter.format_actionable_console_report = format_actionable_console_report
