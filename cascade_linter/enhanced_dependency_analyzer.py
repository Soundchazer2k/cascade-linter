...
class EnhancedDependencyAnalyzer:
    ...
    def detect_missing_docstrings(self) -> Dict[str, List[str]]:
        ...
        missing_docstrings = {"files": [], "count": 0}
    ...
    def analyze_codebase(
        self, include_mypy: bool = True, include_coverage: bool = True
        ...
        if hasattr(result, "metrics") and result.metrics:
            for module_path, metrics in result.metrics.items():
                ...
                if imported_by_count >= self.thresholds.min_imported_by_for_high_risk:
                    ...
                    imports_count > self.thresholds.max_god_module_dependencies
                    or impact_score >= self.thresholds.min_impact_score_critical
                ):
                    risk_category = "HIGH"
                ...
                if imported_by_count > 5:
                    justification = (
                        f"High-impact module used by {imported_by_count} other modules"
                    )
        ...
        enhanced_result = EnhancedAnalysisResult(
            ...
            export_hints={
                "DOT graph": "python -m cascade_linter.cli_enhanced --dependency-analysis --export-graph dependency_graph.dot",
                "CSV export": "python -m cascade_linter.cli_enhanced --dependency-analysis --export-csv dependency_analysis.csv",
                "JSON export": "python -m cascade_linter.cli_enhanced --dependency-analysis --json-pretty > dependency_analysis.json",
            },
            quick_fixes=[],
    ...
    def _detect_architectural_smells(self, result) -> List[ArchitecturalSmell]:
        ...
        if hasattr(result, "priority_errors"):
            for error in result.priority_errors:
                if (
                    error.get("impact_score", 0)
                    > self.thresholds.min_impact_score_critical
                ):
                    smells.append(
                        ArchitecturalSmell(
...
class EnhancedDependencyFormatter:
    ...
    def format_console_report(
        self, result: EnhancedAnalysisResult, show_details: bool = False
    ) -> str:
        ...
        lines.append(
            f"Project: {result.project_path}    Analysis time: {result.analysis_time:.2f}s"
        )
        lines.append(
            f"Files: {result.total_files}    Import relationships: {result.total_imports}    Local modules: {result.local_modules}"
        )
        ...
        if total_modules > 0:
            ...
            lines.append(
                f"{symbols.SUCCESS} PROJECT HEALTH SCORE: {health_score}/100{explanation}"
            )
        lines.append("")
        ...
        if show_details and result.module_breakdowns:
            ...
            medium_modules = [
                (name, breakdown)
                for name, breakdown in sorted_modules
                if getattr(breakdown, "risk_category", "LOW") == "MEDIUM"
            ]
            medium_modules.sort(key=lambda x: x[1].impact_score, reverse=True)
            top_medium_modules = set(
                name for name, _ in medium_modules[:2]
            )  # Top 2 medium modules
            ...
            for module_name, breakdown in sorted_modules:
                ...
                risk_symbol = (
                    ...
                    if risk == "CRITICAL"
                        ...
                        symbols.HIGH
                        if risk == "HIGH"
                        else symbols.INFO if risk == "MEDIUM" else symbols.SUCCESS
                    )
                )
                ...
                if risk in ["CRITICAL", "HIGH"]:
                    ...
                    show_this = (
                        False  # Collapse remaining MEDIUM and all LOW risk modules
                    )
                ...
                if show_this:
                    ...
                    lines.append(
                        f"   • Imports: {breakdown.imports_count}    | Imported by: {breakdown.imported_by_count}    | LOC: {breakdown.lines_of_code}"
                    )
                    lines.append(
                        f"   • Risk: {breakdown.risk_category}    • MyPy Errors: {mypy_errors}"
                    )
                    ...
                    if breakdown.circular_dependencies:
                        lines.append(
                            f"   • Circular deps: {', '.join(breakdown.circular_dependencies[:3])}"
                        )
                    if breakdown.test_coverage is not None:
                        lines.append(
                            f"   • Test coverage: {breakdown.test_coverage:.1f}%"
                        )
            ...
            if remaining_medium > 0 or low_modules_count > 0:
                ...
                lines.append(
                    f"{symbols.SUCCESS} [{collapsed_text}... use --show-all-details to expand]"
                )
                lines.append("")
        ...
        if cycles_found == 0:
            ...
            quick_wins.append(
                f"{symbols.HIGH} {cycles_found} circular dependencies need attention"
            )
        ...
        if mypy_error_total == 0:
            ...
            if mypy_problem_modules:
                ...
                module_short_name = (
                    top_module.split(".")[-1] if "." in top_module else top_module
                )
                quick_wins.append(
                    f"{symbols.HIGH} {top_errors} MyPy errors in {module_short_name} (run with --show-errors)"
                )
            else:
                quick_wins.append(
                    f"{symbols.HIGH} {mypy_error_total} MyPy errors found (run with --show-errors)"
                )
        ...
        if risk_counts["CRITICAL"] == 0:
            ...
            critical_modules = [
                name.split(".")[-1]
                for name, breakdown in result.module_breakdowns.items()
            ...
            quick_wins.append(
                f"{symbols.CRITICAL} {risk_counts['CRITICAL']} critical {module_word} {verb} refactoring ({critical_list})"
            )
        ...
        for module_name, breakdown in result.module_breakdowns.items():
            ...
            if (
                breakdown.lines_of_code > 100
                or breakdown.imports_count > 5
                or breakdown.imported_by_count > 0
        ...
        if estimated_missing > 5:
            potential_quick_wins.append(
                f"{symbols.INFO} Quick Win: ~{estimated_missing} modules likely missing docstrings (use --list-missing-docstrings)"
            )
        elif estimated_missing > 0:
            potential_quick_wins.append(
                f"{symbols.INFO} Quick Win: Add module docstrings for better documentation (~{estimated_missing} missing)"
            )
        ...
        for breakdown in result.module_breakdowns.values():
            if (
                breakdown.imports_count > breakdown.lines_of_code // 20
            ):  # Rough heuristic
                estimated_unused += max(1, breakdown.imports_count // 5)

        if estimated_unused > 5:
            potential_quick_wins.append(
                f"{symbols.SUCCESS} Quick Win: ~{estimated_unused} unused imports estimated (run with --list-unused-imports)"
            )
        ...
        if result.architectural_smells:
            ...
            for smell in result.architectural_smells:
                severity_icon = (
                    ...
                    else symbols.HIGH if smell.severity == "HIGH" else symbols.INFO
                )
                ...
                if smell.affected_modules:
                    lines.append(
                        f"   Affected modules: {', '.join(smell.affected_modules[:3])}"
                    )
                lines.append("")
        ...
        if result.todo_checklist:
            remaining_todos = [
                todo
                for todo in result.todo_checklist
                if todo.get("category") != "Analysis"
            ]
            if remaining_todos:
                # Group by priority
                priority_groups = {}
                ...
                for priority in ["CRITICAL", "HIGH", "MEDIUM", "LOW"]:
                    if priority in priority_groups:
                        priority_icon = (
                            ...
                            else symbols.HIGH if priority == "HIGH" else symbols.INFO
                        )
                        lines.append(f"{priority_icon} {priority} PRIORITY TASKS:")

                        for todo in priority_groups[priority][
                            :2
                        ]:  # Show top 2 per priority
                            lines.append(
                                f"   {symbols.ARROW} {todo.get('task', 'Unknown task')}"
                            )
                            lines.append(
                                f"     Details: {todo.get('details', 'No details')}"
                            )
                            lines.append(
                                f"     Effort: {todo.get('estimated_effort', 'Unknown time')}"
                            )
                            if todo.get("auto_fixable", False):
                                lines.append("     Note: Auto-fixable!")
                            lines.append("")
        ...
        if result.architectural_smells:
            critical_count = len(
                [s for s in result.architectural_smells if s.severity == "CRITICAL"]
            )
            if critical_count > 0:
                lines.append(
                    f"{symbols.HIGH} {critical_count} critical issues need immediate attention"
                )

        if not show_details:
            lines.append(
                f"{symbols.INFO} Run with --show-details for per-module breakdown"
            )
        lines.append(
            f"{symbols.INFO} Run with --json-pretty for machine-readable output"
        )
    ...
    def format_csv_report(self, result: EnhancedAnalysisResult) -> str:
        ...
        lines.append(
            "module_name,short_name,is_snapshot,risk_category,impact_score,imported_by_count,imports_count,lines_of_code,test_coverage,mypy_errors,circular_dependencies,file_path,justification"
        )
        ...
        for module_name, breakdown in sorted_modules:
            ...
            if "backup_before_refactor" in module_name or len(module_name) > 40:
                ...
                is_snapshot = (
                    "TRUE" if "backup_before_refactor" in module_name else "FALSE"
                )
            ...
            circular_deps = (
                "|".join(breakdown.circular_dependencies)
                if breakdown.circular_dependencies
                else "NONE"
            )
            ...
            justification = (
                breakdown.justification.replace(",", ";")
                .replace("\n", " ")
                .replace('"', "'")
            )
    ...
    def export_graphviz_dot(
        self, result: EnhancedAnalysisResult, import_analyzer=None
    ) -> str:
        ...
        for module_name, breakdown in result.module_breakdowns.items():
            if breakdown.imports_count > 0:
                ...
                for target in high_impact_modules[
                    :3
                ]:  # Connect to top 3 high-impact modules
        ...
        lines.append(
            '        legend_critical [label="CRITICAL", fillcolor=red, fontcolor=white];'
        )
        ...
        json_data = {
            ...
            "health_metrics": {
                ...
                "circular_dependencies": len(
                    [
                        m
                        for m in result.module_breakdowns.values()
