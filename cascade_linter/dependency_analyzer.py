...
class ImportGraphBuilder:
    ...
    def analyze_project(self) -> None:
        ...
        print(f"Found {len(self.import_nodes)} import relationships across {len(self.python_files)} files")
    ...
    def _discover_python_files(self) -> None:
        ...
        for py_file in self.project_root.rglob("*.py"):
            ...
            if any(skip in py_file.parts for skip in 
                   ['__pycache__', '.pytest_cache', 'venv', 'env', '.git', 'node_modules']):
                continue
...
class DependencyAnalyzer:
    ...
    def _calculate_module_metrics(self, module_path: str) -> DependencyMetrics:
        ...
        local_imports = {imp for imp in imports 
                        if any(node.imported == imp and node.import_type in ['absolute', 'relative'] 
                              for node in self.graph_builder.import_nodes)}
        local_imported_by = {imp for imp in imported_by 
                            if any(node.importer == imp and node.import_type in ['absolute', 'relative']
                                  for node in self.graph_builder.import_nodes)}
    ...
    def _calculate_impact_scores(self) -> None:
        ...
        max_imported_by = max((m.imported_by_count for m in self.metrics.values()), default=1)
        ...
        for metrics in self.metrics.values():
            ...
            metrics.impact_score = max(0, dependency_factor + depth_factor + complexity_factor + circular_penalty)
    
    def _assign_criticality_rankings(self) -> None:
        """Assign criticality rankings based on impact scores"""
        sorted_metrics = sorted(self.metrics.values(), key=lambda x: x.impact_score, reverse=True)
...
class ErrorPrioritizer:
    ...
    def analyze_error_impact(self, mypy_results: Dict[str, any]) -> Dict[str, ErrorImpactAnalysis]:
        ...
        for file_path, file_results in mypy_results.items():
            ...
            affected_modules = list(self.dependency_analyzer.graph_builder.reverse_graph.get(module_path, set()))
            cascade_impact = len(affected_modules)
            
            # Calculate fix priority score
            fix_priority = self._calculate_fix_priority(error_count, dep_metrics.impact_score, cascade_impact)
    ...
    def _calculate_fix_priority(self, error_count: int, impact_score: float, cascade_impact: int) -> float:
    ...
    def get_top_priority_fixes(self, limit: int = 10) -> List[ErrorImpactAnalysis]:
        ...
        sorted_analyses = sorted(self.error_analyses.values(), 
                               key=lambda x: x.fix_priority_score, reverse=True)
        return sorted_analyses[:limit]


def analyze_project_dependencies(project_path: str, mypy_results: Optional[Dict] = None) -> Dict[str, any]:
    """
    ...
    results = {
        ...
        'analysis_summary': {
            ...
            'circular_dependencies': sum(1 for m in dependency_metrics.values() if m.circular_dependencies)
        },
        ...
        'recommendations': _generate_recommendations(dependency_metrics, priority_fixes)
    }
    
    return results


def _generate_recommendations(dependency_metrics: Dict[str, DependencyMetrics], 
                            priority_fixes: List[ErrorImpactAnalysis]) -> List[str]:
    """Generate actionable recommendations based on analysis"""
    recommendations = []
    
    # Top critical modules
    top_critical = sorted(dependency_metrics.values(), key=lambda x: x.impact_score, reverse=True)[:3]
    ...
    circular_modules = [m for m in dependency_metrics.values() if m.circular_dependencies]
    if circular_modules:
        recommendations.append(
